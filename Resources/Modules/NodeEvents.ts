...
module NodeEvents {
     ...
     */
    export function trigger<T extends TriggerAction>(node: Atomic.Node, eventName: string, data?: T) {
        if (DEBUG) {
     ...
     * @return {Array} an array of all the results, if there are any, otherwise an empty array
     */
    export function triggerSync<T extends TriggerAction>(node: Atomic.Node, eventName: string, data?: T): any[] {
        if (DEBUG) {
            metrics.start(eventName);
        }
